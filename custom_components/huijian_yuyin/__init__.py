...
from .llm_provider import async_setup_llm_providers
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the huijian_yuyin component."""
    ...
    await async_setup_llm_providers(hass)
...
async def async_setup_entry(hass: HomeAssistant, entry: ESPHomeConfigEntry) -> bool:
    ...
    config_type = entry.data.get(CONF_ASSIST_MODE)
...
async def async_unload_entry(hass: HomeAssistant, entry: ESPHomeConfigEntry) -> bool:
    """Unload an huijian_yuyin config entry."""
...
async def async_remove_entry(hass: HomeAssistant, entry: ESPHomeConfigEntry) -> None:
    """Remove an huijian_yuyin config entry."""
...
async def _async_clear_dynamic_encryption_key(
    ...
    try:
        ...
        if not await cli.noise_encryption_set_key(b""):
            _LOGGER.debug(
                "Could not clear dynamic encryption key for device %s",
                entry.unique_id,
            )
            return
    except APIConnectionError as exc:
        _LOGGER.debug(
            "Could not connect to device %s to clear key: %s",
            entry.unique_id,
            exc,
        )
        return
